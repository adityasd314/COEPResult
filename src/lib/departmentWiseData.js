...
const retryRequest = async (mis, maxRetries = 6, delayTime = 5000) => {
  let attempts = 0;
...
const getAllDepartmentData = async () => {
  const department100 = [1, 9, 7, 11, 13];
  const department200 = [3, 10];
...
getAllDepartmentData()
  .then(data => {
    const dataString = JSON.stringify(data, null, 2);  // Convert data to a formatted JSON string
    fs.writeFileSync('departmentData.json', dataString, 'utf-8');  // Save the result to a file
